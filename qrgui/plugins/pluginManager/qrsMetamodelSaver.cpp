...
Id QrsMetamodelSaver::saveNode(qrRepo::RepoApi &repo, const NodeElementType &node, const Id &diagram)
{
	...
	saveQmlGraphics(repo, node, nodeId);
...
void QrsMetamodelSaver::saveQmlGraphics(qrRepo::RepoApi &repo, const NodeElementType &node, const Id &id)
{
	QDomDocument qmlDocument;
	QDomElement graphicsElement = qmlDocument.createElement("graphics");
	qmlDocument.appendChild(graphicsElement);

	QDomElement pictureElement = qmlDocument.createElement("picture");
	pictureElement.setAttribute("sizex", node.size().width());
	pictureElement.setAttribute("sizey", node.size().height());
	pictureElement.appendChild(qmlDocument.createTextNode(node.qml()));
	graphicsElement.appendChild(pictureElement);

	QDomElement labels = qmlDocument.createElement("labels");
	QDomElement ports = qmlDocument.createElement("ports");
	...
	repo.setProperty(id, "shape", qmlDocument.toString(4));
}
